...
# ---------------------------------------------------------------
def augment_transpose(map_state, unit_state, action):
    """
    Augments a sample by transposing the map and updating the unit state and action.
    
    Args:
        map_state (np.ndarray): Array of shape (channels, H, W).
        unit_state (np.ndarray): 1D array (length 10) with the unit state.
        action (int): Expert action.
        
    Returns:
        tuple: (map_state_aug, unit_state_aug, action_aug) where:
            - map_state_aug is the transposed map (shape becomes (channels, W, H)),
            - unit_state_aug is the updated unit state (swapping indices 0↔1 and 2↔3),
            - action_aug is the updated action using the transposed mapping.
    """
    # Transpose the map: swap height and width.
    map_state_aug = np.transpose(map_state, (0, 2, 1))
    
    # Update the unit state: swap the first two entries and the next two entries.
    unit_state_aug = unit_state.copy()
    unit_state_aug[0], unit_state_aug[1] = unit_state[1], unit_state[0]
    unit_state_aug[2], unit_state_aug[3] = unit_state[3], unit_state[2]
    
    # Update the action: for directions (1: up, 2: right, 3: down, 4: left).
    # Under transposition: up -> left, right -> down, down -> right, left -> up.
    transposed_action_map = {0: 0, 1: 4, 2: 3, 3: 2, 4: 1, 5: 5}
    action_aug = transposed_action_map[action]
    return map_state_aug, unit_state_aug, action_aug
...
def process_replay(replay_path, team_id):
    ...
    for step in range(num_steps):
        ...
        if next_obs["team_wins"][0] >= 3 or next_obs["team_wins"][1] >= 3:
            terminated = True
        if step > 0:
        ...
        for unit_id in available_unit_ids:
            ...
            if team_id == 1:
                unit_actual_pos = [23 - unit_pos[1], 23 - unit_pos[0]]
            else:
                unit_actual_pos = unit_pos
            ...
            # Yield the original sample.
            yield step, map_state_np, unit_state, unit_action, terminated

            # Also yield the augmented sample (transposed map, updated unit state and action).
            aug_map_state, aug_unit_state, aug_action = augment_transpose(map_state_np, unit_state, unit_action)
            yield step, aug_map_state, aug_unit_state, aug_action, terminated
...
# ---------------------------------------------------------------
def process_all_replays(replay_folder, hdf5_filename, batch_size=10000):
    # Open file in append mode. If it doesn't exist, it will be created.
    with h5py.File(hdf5_filename, 'a') as hf:
        # Check if datasets already exist.
        if "unique_map_states" in hf:
            unique_map_ds = hf["unique_map_states"]
            unit_states_ds = hf["unit_states"]
            map_ids_ds = hf["map_ids"]
            actions_ds = hf["actions"]
        ...
        map_shape = None  # e.g., (channels, H, W)
        unit_shape = (10,)
        player_0_num = 0
        player_1_num = 0
        ...
        for file in os.listdir(replay_folder):
            if file.endswith(".json"):
                ...
                if replay is None:
                    continue
                team_ids = []
                if replay["info"]["TeamNames"][0] == "aDg4b":
                    team_ids.append(0)
                    player_0_num += 1
                if replay["info"]["TeamNames"][1] == "aDg4b":
                    team_ids.append(1)
                    player_1_num += 1
                # For each replay, keep a mapping from its (team_id, step) to a global map id.
                replay_map_to_global = {}
                # Process each team in the replay.
                for team_id in team_ids:
                    for step, map_state, unit_state, unit_action, terminated in process_replay(replay_path, team_id=team_id):
                        if terminated:
                            break
                        ...
                        key = (team_id, step, hash(map_state_np.tobytes()))
                        ...
                        if sample_count % batch_size == 0:
                            # Create the datasets on the first flush if they don't exist.
                            ...
                            # Flush the unique map states buffer.
...
if __name__ == '__main__':
    ...
    with h5py.File("training_samples.hdf5", "r") as hf:
        # Access the correct dataset names
