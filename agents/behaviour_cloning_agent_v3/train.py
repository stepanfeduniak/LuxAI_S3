...
def train_behavior_cloning(
    ...
    for epoch in range(num_epochs):
        ...
        for i, (unit_states, map_states, actions) in enumerate(dataloader):
            ...
            logits = model(map_states,unit_states)
            # Create a categorical distribution for computing entropy
            #dist = torch.distributions.Categorical(logits=logits)
            # Compute standard cross-entropy loss
            loss_ce = criterion(logits, actions)
            # Subtract a small entropy bonus to encourage some exploration
            #entropy = dist.entropy().mean()
            loss = loss_ce #- entropy_coeff * entropy
            loss.backward()
            optimizer.step()
            ...
            if i % 40 == 0:
                torch.save(model.state_dict(), "bc_model_resnet.pth")
